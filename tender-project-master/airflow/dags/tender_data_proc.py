...
BUCKET_NAME = "dl-eu-pub-tender-2023"
...
PYSPARK_JOB = {
    ...
    "pyspark_job": {
        ...
        "jar_file_uris": [
            "gs://spark-lib/bigquery/spark-bigquery-with-dependencies_2.12-0.24.2.jar"
        ],
    },
}
...
def create_data_links():
    df_country_abrvs = pd.read_csv(
        f"{AIRFLOW_HOME}/data/countries_abr.csv", index_col="ISO2"
    )
...
with DAG(
    ...
    check_data_availability = HttpSensor(
        task_id="check_data_availability", http_conn_id="data_page", endpoint="/"
    )
    ...
    download_data = PythonOperator(
        task_id="download_data", python_callable=create_data_links
    )
...
(
    [check_abr_availability, check_data_availability]
    >> download_abbrevs
    >> download_data
    >> upload_files
    >> create_cluster
